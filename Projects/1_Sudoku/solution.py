...
dg_unit = [[rows[i] + cols[i] for i in range(len(rows))], [rows[::-1][i] + cols[i] for i in range(len(rows))]]
unitlist = row_units + column_units + square_units + dg_unit
...
# help from reviwer...
    for unit in unitlist:
        ...
        for pair in poss_twins:
            ...
            if values[bx_1] == values[bx_2]:
                for box in unit:
                    # Eliminate the naked twins as possibilities for peers
                    if box != bx_1 and box != bx_2:
                        for remove_val in values[bx_1]:
                            values[box] = values[box].replace(remove_val,'')
    return values
