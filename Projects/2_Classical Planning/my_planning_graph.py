...
class ActionLayer(BaseActionLayer):
    ...
    def _inconsistent_effects(self, actionA, actionB):
        ...
        area = product(actionA.effects, actionB.effects)
        return any(~acA == acB for acA, acB in area)
    ...
    def _interference(self, actionA, actionB):
        ...
        """
        # TODO: implement this function
        area = chain(product(actionA.effects, actionB.preconditions), product(actionB.effects, actionA.preconditions))
        return any(~ef == prd for ef, prd in area)
    ...
    def _competing_needs(self, actionA, actionB):
        ...
        --------
        layers.ActionNode
        layers.BaseLayer.pre
        """
        # TODO: implement this function
        area = product(actionA.preconditions, actionB.preconditions)
        return any(self.parent_layer.is_mutex(prcA, prcB) for prcA, prcB in area)
...
class LiteralLayer(BaseLiteralLayer):
    def _inconsistent_support(self, literalA, literalB):
        ...
        """
        # TODO: implement this function
        area = product(self.parents[literalA], self.parents[literalB])
        return all(self.parent_layer.is_mutex(a,b) for a,b in area)
...
class PlanningGraph:
    ...
    def h_levelsum(self):
        ...
        For example, if Goal_1 first appears in level 0 of the graph (i.e.,p
        ...
        while not self._is_leveled:
            while goals:
                gl = goals[-1]
                goals = goals[0:-1]
                if gl in self.literal_layers[level]:
                    cost += level
                else:
                    w_area.append(gl)
            goals, w_area = w_area, goals
            self._extend()
            level += 1
        if goals:
            return level-1
    ...
    def h_setlevel(self):
        ...
        level = 0
        while not self._is_leveled:
            lt = self.literal_layers[level]
            if self.goal.issubset(lt):
                if not any(lt.is_mutex(a, b) for a, b in combinations(lt, 2)):
                    return level
            self._extend()
            level += 1
        return level-1
