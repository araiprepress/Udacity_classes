...
_BLANK_BOARD = 0 #41523161203939122082683632224299007
row = ((1<<_WIDTH) - 1)#2047
...
S, N, W, E = -_WIDTH - 2, _WIDTH + 2, 1, -1#(-13, 13, 1,-1)

class Action(IntEnum):
    """ The eight L-shaped steps that a knight can move in chess """
    NNE = N+N+E  # north-northeast (up, up, right)#25
    ENE = E+N+E  # east-northeast (right, right, up)#11
    ESE = E+S+E  # east-southeast (right, right, down)#-15
    SSE = S+S+E  # south-southeast (down, down, right)#-27
    SSW = S+S+W  # south-southwest (down, down, left)#-25
    WSW = W+S+W  # west-southwest (left, left, down)#-11
    WNW = W+N+W  # west-northwest (left, left, up)#15
    NNW = N+N+W  # north-northwest (up, up, left)#27

_ACTIONSET = set(Action)  # used for efficient membership testing
# print(_ACTIONSET)
#{<Action.SSE: -27>, <Action.SSW: -25>, <Action.ENE: 11>, <Action.WNW: 15>,
# <Action.ESE: -15>, <Action.WSW: -11>, <Action.NNE: 25>, <Action.NNW: 27>
...
# <Action.ESE: -15>, <Action.WSW: -11>, <Action.NNE: 25>, <Action.NNW: 27>
    ...
    def result(self, action):
        ...
        if not (self.board0 & (1 << player_location)):
